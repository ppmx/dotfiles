...
-- 	   		              			          |___/                          --
--                                                                           --
--   Keys for controlling the window manager environment, and associated     --
--   helper functions.                                                       --
-------------------------------------------------------------------------------
...
local function rename_tag(tag, title)
	-- extract current tag index:
	_, _, current_index = string.find(tag.name, "^%[([%d]+)")

	-- rename, and if title is not null then append it as well:
	tag.name = "[" .. current_index
...
globalkeys = gears.table.join(
	...
	awful.key(
		{ modkey }, "l",
		function () awful.spawn("/usr/bin/pcmanfm") end,
	...
	awful.key(
		{}, "XF86MonBrightnessUp",
		function () awful.util.spawn("light -A 5", false) end
	),

	awful.key(
		{}, "XF86MonBrightnessDown",
		function () awful.util.spawn("light -U 5", false) end
	),
	...
	awful.key(
		{ modkey }, "x",
		function () awful.spawn("screenlock") end,
		{description = "lock the screen", group = "launcher"}
	),
	...
	awful.key(
		{ modkey }, "r", function () awful.spawn("rofi -show run") end,
		{description = "run rofi launcher", group = "launcher"}
	),

	awful.key(
		{ modkey }, "w", function () awful.spawn("rofi -show window") end,
	...
	awful.key(
		...
		function ()
			...
			exe_callback = function (s) rename_tag(awful.tag.selected(), s) end
